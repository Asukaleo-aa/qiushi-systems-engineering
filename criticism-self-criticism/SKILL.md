...
本 skill 与系统工程层的 `feedback-and-revision-loop` 构成**人-系统双重反馈闭环**：

| 维度 | `criticism-self-criticism`（本 skill，人维度） | `feedback-and-revision-loop`（系统维度） |
|------|----------------------------------------------|----------------------------------------|
| 审视对象 | Agent 的**工作方法和决策质量** | 系统的**行为偏差和模型误差** |
| 核心问题 | "我哪里做得不对？我跳过了什么步骤？" | "系统哪里偏离了预期？根因在哪个层次？" |
| 分析方法 | 自我批评检查清单 → 具体行为归因 | 五层根因分析法（Why1→Why5） |
| 修正方向 | "下次我会……"（方法论层面的改进） | 短/中/长期三级修正 + 认知更新 |
| 方法论来源 | 毛泽东《整顿党的作风》《论联合政府》 | 钱学森控制论 + 反馈原理 |
...
[阶段工作完成]
    ↓
criticism-self-criticism （审视"我做得怎么样"）
    ↓ 产出：工作审视报告
    ├─ 人的失误（跳过验证/调查不足/分心）→ 下次重点关注
    └─ 如果同时发现系统偏差（结果与预期不符）→ 触发 ↓
feedback-and-revision-loop （审视"系统哪里偏离了"）
    ↓ 产出：反馈修正报告
    └─ 根因如果在哲学/理论层 → 返回 systems-thinking-framework
       根因如果在方法/执行层 → 返回对应 skill 修正
```

**合并审视：** 当两个 skill 同时触发时，可以将审视报告中的"发现的问题"与 feedback-and-revision-loop 的"偏差分析矩阵"对照：人的失误是否导致了系统偏差？系统偏差是否反映了认知盲区？

---

## 原著依据

本 skill 的方法论来自毛泽东关于批评与自我批评的多篇论述：
- 《整顿党的作风》（1942年）："惩前毖后，治病救人"
- 《论联合政府》（1945年）："房子是应该经常打扫的"
- 反对"残酷斗争、无情打击"，主张"团结——批评——团结"
