...
| 平台 | 安装方式 | 自动注入 |
|------|---------|---------|
| **OpenHanako** | `bash .openhanako/install.sh` | ✅ SessionStart 自动加载 |
| **Claude Code** | `claude plugin add .` | ✅ SessionStart hook + 28 commands |
| **Cursor** | 注册插件路径 | ✅ SessionStart hook |
| **Reasonix** | `powershell -File .reasonix/setup.ps1` | ✅ 技能索引发现 |
| **Trae** | 复制 `.trae/skills/` 或导入 `skills.zip` | 手动 |
| **OpenCode** | 见 [.opencode/INSTALL.md](.opencode/INSTALL.md) | ✅ 技能自动匹配 |
| **Codex** | 见 [.codex/INSTALL.md](.codex/INSTALL.md) | 手动 |

详见 [INSTALL.md](INSTALL.md)（含平台能力对比矩阵）。

## 🧪 形式化层

方法论的结构已被翻译为控制论数学等价物，并实现为可计算原型。见 [formalization.md](formalization.md) 和独立仓库 [qiushi-formalization](https://github.com/Asukaleo-aa/qiushi-formalization)。

## 版本

- v3.2.3：Trae 平台适配——28 skill 英文化 + skills.zip 打包
- v3.2.2：plugin.json schema 修正 + marketplace.json + author 字段规范化
- v3.2.1：多平台完整部署（6 平台）+ Scenario 6 标定结果入文档
