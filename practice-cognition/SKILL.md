...
本 skill 是系统工程"实践检验环节"的认识论基础。在系统工程侧，有三个 skill 直接实现本 skill 的核心理念：

| 本 skill 的核心理念 | 系统工程侧的技术实现 |
|-------------------|--------------------|
| "实践是检验真理的唯一标准" | `simulation-validation-cycle`：用可重复的仿真实验检验模型的正确性 |
| "感性认识→理性认识"的飞跃 | `quantitative-modeling-workflow`：从定性假设飞跃到定量模型 |
| "循环往复以至无穷" | `feedback-and-revision-loop`：偏差→根因→修正→再检验的持续闭环 |
| "真理的标准只能是社会实践" | 三 skill 联合：建模→仿真→反馈修正→再建模 |

**联合调用模式：**
- 当进入"实践验证"阶段时，先由本 skill 宣告当前处于实践认识循环的哪个阶段
- 再由 `simulation-validation-cycle` 执行具体的仿真验证（正常/边界/异常/压力场景）
- 如果仿真通过 → 本 skill 的第四步"认识基本正确" → 进入下一轮
- 如果仿真未通过 → 本 skill 的第四步"认识有误" → 触发 `feedback-and-revision-loop` 进行根因分析
- 修正后（`quantitative-modeling-workflow` 修改模型）→ 回到本 skill 第三步"回到实践中验证"
- 整个过程中，`criticism-self-criticism` 负责审视工作质量，`feedback-and-revision-loop` 负责修正系统偏差——两者形成"人+系统"双重反馈环路

---

## 原著依据

本 skill 的方法论全部来自毛泽东《实践论》（1937年7月），包括：
- 实践是认识的来源（"你要知道梨子的滋味，你就得变革梨子，亲口吃一吃"）
- 感性认识→理性认识的飞跃
- 理性认识→实践的飞跃（"实践是检验真理的唯一标准"）
- 实践→认识→再实践的螺旋上升（"循环往复以至无穷"）
