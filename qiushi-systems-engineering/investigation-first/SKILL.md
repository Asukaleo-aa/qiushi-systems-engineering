...
- **第一性原理分析**：调查之前需明确"我到底要调查什么本质问题"——`first-principles-analysis` 剥离表象→暴露核心，本 skill 再深入一线获取第一手事实。避免带着错误的假设框架去收集数据。
...
本 skill 与系统工程层的 `problem-domain-investigation` 构成**双轨调查**：

| 维度 | `investigation-first`（本 skill） | `problem-domain-investigation` |
|------|---------------------------------|-------------------------------|
| 方法论来源 | 毛泽东《反对本本主义》 | 钱学森系统工程方法论 |
| 核心方法 | 深入一线、亲手调查、七项注意 | 知识地图、多源交叉验证、信息可靠性标记 |
| 输出重点 | 调查提纲 → 事实清单 → 实事求是结论 | 领域调研报告：已知/未知/推断/存疑/分歧 |
| 适用场景 | 需要哲学方法保证不脱离实际时 | 需要工程方法保证结构化覆盖时 |

**联合调用协议：**
1. 先由 `investigation-first` 出调查提纲（哲学方法确保方向正确）
2. 再由 `problem-domain-investigation` 按四层知识地图填充结构化内容（工程方法确保覆盖完整）
3. 两方结论交叉验证：`investigation-first` 的"实事求是结论" 与 `problem-domain-investigation` 的"调研结论"对照，不一致处标注 `[分歧]` 并触发 `meta-synthesis-engine` 或 `contradiction-analysis`
