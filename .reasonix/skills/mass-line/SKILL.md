...
本 skill 的"收集→集中→返回→检验→再收集"循环，在系统工程层有两个直接的实现载体：

| 本 skill 的阶段 | 系统工程侧的实现 | 联动方式 |
|---------------|----------------|---------|
| 收集（从群众中来） | `investigation-first`（工程扩展模式） | 多源信息采集 → 信息可靠性标记 |
| 集中（系统化研究） | `meta-synthesis-engine` | 本 skill 的"集中"产出（分散意见→系统化方案）作为 meta-synthesis 第一阶段"信息汇聚"的输入 |
| 返回（到群众中去） | `simulation-validation-cycle` | 方案提交给仿真实验检验（"在实践中考验这些意见是否正确"） |
| 检验（行动中验证） | `feedback-and-revision-loop` | 方案执行后的偏差反馈（"在群众行动中考验"） |
| 再收集（新循环） | `practice-cognition` | 新一轮认识循环的起点 |

**核心耦合：** `mass-line` 的"集中"与 `meta-synthesis-engine` 的综合集成本质上是同一过程的两个视角——前者是组织方法论（怎么把分散的意见凝聚成统一的行动方案），后者是技术方法论（怎么把定性和定量的多源信息综合为统一判断）。两者在同一问题上的联合调用可以形成"群众经验 + 专家判断 + 定量模型"的三位一体综合。

---

## 原著依据

本 skill 的方法论全部来自毛泽东《关于领导方法的若干问题》（1943年6月），核心原则：
- "从群众中来，到群众中去"
- 集中→返回→检验→再集中的无限循环
- 一般号召与个别指导相结合
- 反对命令主义和尾巴主义
