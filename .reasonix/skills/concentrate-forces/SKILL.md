...
本 skill 与系统工程层的 `engineering-orchestration` 构成**战略-战术双层执行体系**：

| 维度 | `concentrate-forces`（本 skill，战略层） | `engineering-orchestration`（战术层） |
|------|----------------------------------------|-------------------------------------|
| 核心问题 | 同时面对多个任务时，**攻哪一个**？ | 任务确定了之后，**怎么拆解和编排**？ |
| 方法论来源 | 毛泽东十大军事原则 | 钱学森系统工程 + 聂荣臻两弹一星组织经验 |
| 输出 | 主攻目标宣告 + 任务优先级矩阵 | WBS + 关键路径 + 质量门 + 风险矩阵 |
| 时间尺度 | 当前战役（短期聚焦） | 全程项目周期（WBS覆盖全貌） |

**联合调用协议：**
1. `engineering-orchestration` 先出完整的 WBS 和关键路径分析
2. `concentrate-forces` 在关键路径上选择**当前阶段**的主攻方向
3. 每通过一个质量门（Gate），重新调用 `concentrate-forces` 评估：
   - 当前战役是否已完成？（"断其一指"的信号是否触发）
   - 是否需要转向下一个目标？
   - 原有 WBS 的依赖关系是否因前序任务的结果而改变？
4. 阶段性复盘同时触发 `criticism-self-criticism`（审视工作质量）和 `feedback-and-revision-loop`（修正编排偏差）

---

## 原著依据

本 skill 的方法论来自毛泽东十大军事原则（1947年）和《中国革命战争的战略问题》（1936年），核心原则：
- "集中优势兵力，各个歼灭敌人"
- "伤其十指不如断其一指"
- "不打无准备之仗，不打无把握之仗"
