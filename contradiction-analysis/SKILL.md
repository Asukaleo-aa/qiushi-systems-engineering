...
- 多源信息矛盾 → 综合集成（调用 `meta-synthesis-engine`；本 skill 的矛盾辨识结果作为 meta-synthesis 第二阶段的输入）
...
- **第一性原理分析**：矛盾分析前需先理解问题的本质——`first-principles-analysis`（"最基本的事实是什么"）→ 本 skill（"核心的张力在哪里"）。不被表象矛盾迷惑，先确定什么是不可再分的根基。
...
- **综合集成引擎**（`meta-synthesis-engine`）：当矛盾分析面对多个信息源的矛盾结论时，本 skill 的矛盾清单→性质判定→主要矛盾 直接作为 meta-synthesis 第二阶段的输入。反过来，meta-synthesis 汇总的多源证据也可以触发本 skill 重新评估矛盾格局。
- **系统思维框架**（`systems-thinking-framework`）：矛盾分析聚焦系统内部的张力——组分间的 +1/-1 关联矩阵就是矛盾图谱的另一种表达。两 skill 可联合：矛盾分析定性找主次，系统思维定量画关联。
- **反馈修正循环**（`feedback-and-revision-loop`）：矛盾转化（第六步）的监控结果直接触发 feedback-and-revision-loop，形成"识别矛盾→解决矛盾→发现新矛盾"的动态回路。

---

## 原著依据

本 skill 的方法论全部来自毛泽东《矛盾论》（1937年8月），包括但不限于：
- 矛盾的普遍性与特殊性
- 主要矛盾与次要矛盾的区分（"捉住了这个主要矛盾，一切问题就迎刃而解了"）
- 矛盾的主要方面与次要方面
- 对抗性矛盾与非对抗性矛盾的区分
- 矛盾诸方面的同一性与斗争性
...
**场景**：你接手一个遗留项目，用户抱怨「功能开发太慢，bug 太多，架构需要重构」。三个问题互相牵制，不知道先做什么。

**触发**：多个冲突因素、trade-off、主次不清 → 调用 contradiction-analysis。

**流程**：
1. **识别矛盾**：开发速度 vs 代码质量、新功能 vs 重构、用户期望 vs 技术债务
2. **判定主要矛盾**：代码质量差→每次加功能都引入新 bug→修复 bug 又拖慢开发→质量差是根源。主要矛盾 = 代码质量 vs 开发速度，质量差是主要方面
3. **判定矛盾性质**：非对抗性——质量提升最终会让开发加速
4. **选择处理方式**：不是「先重构再开发」也不是「不管质量赶功能」，而是「每次开发新功能时强制提升相关模块质量」
5. **输出**：⭐ 主要矛盾 = 代码质量 vs 开发速度，主攻方向 = 渐进式质量提升，暂时放缓纯重构需求
